...
public class DocGui {
    ...
    static MyFrame queryDocGui;
    static MyFrame uploadDocGui;
    static MyFrame downloadDocGui;
    static JFileChooser uploadChoose = new JFileChooser(".");
    static JFileChooser downloadChoose = new JFileChooser(".");

    static {
        // 初始化
        DataProcessing.init();
        docManageGui = newDocManageGui();
        queryDocGui = newQueryDocGui();
        uploadDocGui = newUploadDocGui();
        downloadDocGui = newDownloadDocGui();
        // 文件选择器
    ...
    public static MyFrame newDocManageGui() {
        ...
        int be = 0, panelNum = 3;
        // 获取所有档案数据，以此创建表格
        Vector<Vector<String>> rowsData = DataProcessing.listDocToVector();
        ...
        jFrame.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                System.out.printf("{ msg:docGui/newDocManageGui/activated, nowUser:%s }\n", user);
                ...
                // 设置非录入员无法上传文件
                uploadButton.setEnabled(user.getRole().equals(DataProcessing.Role.OPERATOR.getName()));
            }

            @Override
            public void windowClosed(WindowEvent e) {
                System.out.printf("{ msg:docGui/newDocManageGui/closed, nowUser:%s }\n", user);
                // 关闭窗口时，检查子窗口是否已关闭
     ...
     * @throws
     */
    public static MyFrame newQueryDocGui() {
        ...
        confirmButton.addActionListener(e -> {
            ...
            System.out.printf("{ msg:docGui/newQueryDocGui/confirmButton, nowUser:%s, id:%s, searchDoc:%s }\n", user, id, doc);
     ...
     * @throws
     */
    public static MyFrame newUploadDocGui() {
        ...
        pathButton.addActionListener(e -> {
            ...
            System.out.printf("{ msg:docGui/newUploadDocGui/pathButton, nowUser:%s, result:%d, file:%s }\n", user, result, file);
        });
        confirmButton.addActionListener(e -> {
            String id = idField.getText();
            String description = descriptionField.getText();
            String path = pathField.getText();
            System.out.printf("{ msg:docGui/newUploadDocGui/confirmButton, nowUser:%s, id:%s, description:%s, path:%s }\n", user, id, description, path);
            ...
            } else if ( !AbstractUser.copyFile( file.getPath() , "%s\\%s-%s".formatted(AbstractUser.uploadPath , id , DataProcessing.getFilename(path) ) ) ) {
                jFrame.showTip("档案上传失败");
            } else if (!DataProcessing.insertDoc(id, user.getName(),
     ...
     * @throws
     */
    public static MyFrame newDownloadDocGui() {
        ...
        jFrame.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                ...
                System.out.printf("{ msg:docGui/newDownloadDocGui/activated, nowUser:%s, row:%d }\n", user, row[0]);
                if (row[0] == -1) {
                ...
                System.out.printf("{ msg:docGui/newDownloadDocGui/activated/getValue, id:%s, filename:%s }\n", id, filename);
                int option = jFrame.showConfirm("是否确认下载该文档？");
                System.out.printf("{ msg:docGui/newDownloadDocGui/activated/confirm, option:%d }\n", option);
                if (option == JOptionPane.OK_OPTION) {
                    int result = downloadChoose.showOpenDialog(jFrame);
                    File file = downloadChoose.getSelectedFile();
                    if (result == JFileChooser.APPROVE_OPTION && file != null) {
                        if (AbstractUser.copyFile( "%s\\%s-%s".formatted(AbstractUser.uploadPath , id , filename ) , "%s\\%s".formatted(file.getPath(), filename) ) ){
                            jFrame.showTip("下载成功");
                            jFrame.dispose();
                        } else {
                    ...
                    System.out.printf("{ msg:docGui/newDownloadDocGui/activated/confirm, result:%d, file:%s }\n", result, file);
                }
                row[0] = -1;
                jFrame.dispose();
            }
