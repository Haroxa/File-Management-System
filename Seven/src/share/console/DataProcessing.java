...
public class DataProcessing {
     ...
     * @throws
     */
    public static boolean deleteDoc(String sid) {
        try {
            checkDB();
            preparedStatement = connection.prepareStatement("delete from doc_info where sid = ?");
            preparedStatement.setString(1,sid);
            resultInt = preparedStatement.executeUpdate();
            return resultInt != 0;
        } catch (Exception e) {
            System.out.printf("{ msg:delete doc error, sid:%s, error:%s }\n", sid, e.getMessage());
        }
        return false;
    }
    ...
    public static boolean deleteUser(String name) {
        try {
            ...
            System.out.printf("{ msg:delete user error, name:%s, error:%s }\n", name, e.getMessage());
        }
        return false;
    }
