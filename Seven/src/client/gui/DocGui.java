...
public class DocGui {
    ...
    static final Dimension BUTTON_DIMENSION = new Dimension(112, 25);
    ...
    public static MyFrame newDocManageGui() {
        ...
        MyFrame uploadDocGui = newUploadDocGui();
        MyFrame downloadDocGui = newDownloadDocGui(jTable);
        MyFrame deleteDocGui = newDeleteDocGui(jTable);
        queryDocGui.parentGui = uploadDocGui.parentGui
                = downloadDocGui.parentGui = deleteDocGui.parentGui = jFrame;
        ...
        jFrame.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                ...
                deleteButton.setEnabled(jFrame.user.getRole().equals(DealClient.Role.OPERATOR.getName()));
            }
        ...
        deleteButton.addActionListener(e -> jFrame.setCurrentGui(deleteDocGui));
        return jFrame;
    }
...
//                    tableModel.insertRow(tableModel.getRowCount(), doc.toVector());
     ...
     * @throws
     */
    public static MyFrame newDeleteDocGui(JTable jTable) {
        MyFrame jFrame = new MyFrame("删除档案");
        // 设置窗口为无边框，完全透明
        jFrame.setUndecorated(true);
        jFrame.setOpacity(0);
        final int[] row = {-1};
        ...
        jFrame.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                ...
                String id = (String) jTable.getValueAt(row[0], 0);
                String creator = (String) jTable.getValueAt(row[0], 1);
                String filename = (String) jTable.getValueAt(row[0], 3);
                System.out.printf("{ gui-msg:docGui/newDeleteDocGui/activated/getValue, id:%s, filename:%s }\n", id, filename);
                int option = jFrame.showConfirm("是否确认删除该文档？");
                System.out.printf("{ gui-msg:docGui/newDeleteDocGui/activated/confirm, option:%d }\n", option);
                if (option == JOptionPane.OK_OPTION) {
                    if (!creator.equals(jFrame.user.getName())) {
                        jFrame.showTip("非当前用户上传，无法删除");
                    } else if (DealClient.deleteDoc(id,filename)) {
                        jFrame.showTip("删除成功");
                        jFrame.dispose();
                    } else{
                        jFrame.showWarn("文件不存在或删除失败");
                    }
                }
                row[0] = -1;
                jFrame.dispose();
