...
public class LoginGui {
    ...
    public static MyFrame newLoginGui() {
        ...
        loginButton.addActionListener(e -> {
            ...
            System.out.printf("{ gui-msg:userGui/newLoginGui/login, name:%s, password:%s }\n", name, password);
            if(!AbstractUser.checkName(name)){
                jFrame.showTip("用户名格式错误");
            } else if (!AbstractUser.checkPassword(password)) {
                jFrame.showTip("密码格式错误");
            }else{
                AbstractUser user = DealClient.verifyUser(name, password);
                if (user != null) {
                    jFrame.showTip("登录成功");
                    jFrame.dispose();
                    jFrame.user = user;
                    MainGui.start(jFrame);
                } else {
