...
public class UserGui {
    ...
    static MyFrame queryUserGui;
    static MyFrame insertUserGui;
    static MyFrame updateUserGui;
    static MyFrame deleteUserGui;
    static MyFrame userManageGui;
    static int updateRowStore;
    static {
        // 初始化
        DataProcessing.init();
        userManageGui = newUserManageGui();
        queryUserGui = newQueryUserGui();
        insertUserGui = newInsertUserGui();
        updateUserGui = newUpdateUserGui();
        deleteUserGui = newDeleteUserGui();
    }
    ...
    public static MyFrame newUserManageGui() {
        ...
        int be = -1, panelNum = 1;
        // 获取所有用户数据，以此创建表格
        Vector<Vector<String>> rowsData = DataProcessing.listUserToVector();
        tableModel = MyFrame.getNonEditableTableModel(rowsData, COLUMN_DATA);
        ...
        jFrame.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                System.out.printf("{ msg:userGui/newUserManageGui/activated, nowUser:%s }\n", user);
            ...
            public void windowClosed(WindowEvent e) {
                System.out.printf("{ msg:userGui/newUserManageGui/closed, nowUser:%s }\n", user);
                // 关闭窗口时，检查子窗口是否已关闭
        ...
        updateButton.addActionListener(e -> {
            // 只有选中用户，并点击修改，才会呈现此用户信息
            updateRowStore = jTable.getSelectedRow();
            jFrame.setCurrentGui(updateUserGui);
        });
     ...
     * @throws
     */
    public static MyFrame newQueryUserGui() {
        ...
        confirmButton.addActionListener(e -> {
            ...
            AbstractUser u = DataProcessing.searchUser(name);
            System.out.printf("{ msg:userGui/newQueryUserGui/confirmButton, nowUser:%s, name:%s, searchUser:%s }\n", user, name, u);
    ...
    public static MyFrame newInsertUserGui() {
        ...
        confirmButton.addActionListener(e -> {
            ...
            System.out.printf("{ msg:userGui/newInsertUserGui/confirmButton, nowUser:%s, name:%s, role:%s }\n", user, name, role);
            if (!AbstractUser.checkName(name)) {
                jFrame.showTip("用户名格式错误");
            } else if (!DataProcessing.insertUser(name, DataProcessing.getDefaultPassword(), role)) {
     ...
     * @throws
     */
    public static MyFrame newUpdateUserGui() {
        ...
        jFrame.addWindowListener(new WindowAdapter() {
            @Override
            public void windowActivated(WindowEvent e) {
                System.out.printf("{ msg:userGui/newUpdateUserGui/activated, nowUser:%s, updateRowStore:%d }\n", user, updateRowStore);
                if (updateRowStore == -1) {
                ...
                System.out.printf("{ msg:userGui/newUpdateUserGui/activated, nowUser:%s, row:%d, rowData:%s }\n", user, updateRowStore, Arrays.toString(rowData));
                // 设置无法修改自己的角色
                comboBox.setEnabled(!rowData[0].equals(user.getName()));
            }
        });
        ...
        passwordButton.addActionListener(e -> {
            ...
            if (option == JOptionPane.OK_OPTION) {
                rowData[1] = DataProcessing.getDefaultPassword();
        ...
        confirmButton.addActionListener(e -> {
            ...
            System.out.printf("{ msg:userGui/newUpdateUserGui/confirmButton, nowUser:%s, rowData:%s }\n", user, Arrays.toString(rowData));
     ...
     * @throws
     */
    public static MyFrame newDeleteUserGui() {
        ...
        jFrame.addWindowListener(new WindowAdapter() {
            ...
            public void windowActivated(WindowEvent e) {
                ...
                System.out.printf("{ msg:userGui/newDeleteUserGui/activated, nowUser:%s, row:%s }\n", user, Arrays.toString(row));
                if (row[0] == -1) {
                    jFrame.showTip("请先选中指定行，再进行操作");
                    jFrame.dispose();
                    return;
                ...
                System.out.printf("{ msg:userGui/newDeleteUserGui/activated/getValue, name:%s }\n", name);
                ...
                int option = jFrame.showConfirm("是否确认%s用户？".formatted(name.equals(user.getName()) ? "注销当前" : "删除该" ));
                System.out.printf("{ msg:userGui/newDeleteUserGui/activated/confirm, option:%d }\n", option);
                if (option == JOptionPane.OK_OPTION) {
